...
const C = {
  ...
  ink: '#1D1D1D', muted: '#8A8A85', muted2: '#4A4A46',
  border: '#EBE8E0', border2: '#D9D5C9',
  white: '#FFFFFF', bg: '#FAFAF7', paper: '#F4F2EC',
  paperDark: '#F4F2EC',
  teal: '#0F6E56', tealLight: '#E1F5EE',
  blue: '#185FA5', blueLight: '#E6F1FB',
  warn: '#854F0B', warnLight: '#FAEEDA',
  danger: '#991F1F', dangerLight: '#FCEBEB',
  info: '#185FA5',
}

function FSelect({ label, required, value, onChange, placeholder, options }) {
  return (
    <div style={{ marginBottom: 18 }}>
      <div style={{ fontSize: 11, fontWeight: 700, color: C.muted, letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 6 }}>
        {label}{required && <span style={{ color: C.accent }}> *</span>}
      </div>
      <select value={value} onChange={e => onChange(e.target.value)}
        style={{ width: '100%', padding: '10px 13px', border: `1.5px solid ${C.border}`, borderRadius: 8, fontSize: 13, fontFamily: 'inherit', color: C.ink, background: C.white, outline: 'none', boxSizing: 'border-box' }}>
...
function FInput({ label, required, hint, value, onChange, placeholder }) {
  return (
    <div style={{ marginBottom: 18 }}>
...
function FTextarea({ label, required, hint, value, onChange, placeholder, rows }) {
  return (
    <div style={{ marginBottom: 18 }}>
      <div style={{ fontSize: 11, fontWeight: 700, color: C.muted, letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 4 }}>
        {label}{required && <span style={{ color: C.accent }}> *</span>}
      ...
      <textarea value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} rows={rows || 4}
        style={{ width: '100%', padding: '10px 13px', border: `1.5px solid ${C.border}`, borderRadius: 8, fontSize: 13, fontFamily: 'inherit', color: C.ink, background: C.white, outline: 'none', resize: 'vertical', lineHeight: 1.6, boxSizing: 'border-box' }} />
    </div>
  )
}

function Chip({ label, selected, onClick, col }) {
  const bg = selected ? (col === 'teal' ? C.teal : col === 'warn' ? C.warn : C.accent) : C.paper
  const color = selected ? C.white : C.muted2
  return (
    <button onClick={onClick}
      style={{ padding: '7px 14px', borderRadius: 20, border: `1.5px solid ${selected ? 'transparent' : C.border}`, background: bg, color, fontSize: 13, fontWeight: selected ? 600 : 400, cursor: 'pointer', fontFamily: 'inherit', marginRight: 7, marginBottom: 7, transition: 'all 0.12s' }}>
      {label}
    </button>
  )
}

function PainCheckbox({ label, eg, selected, onToggle }) {
  return (
    <div onClick={onToggle}
      style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '9px 12px', borderRadius: 8, border: `1.5px solid ${selected ? C.accent : C.border}`, background: selected ? '#FFF3F3' : C.white, cursor: 'pointer', marginBottom: 7, transition: 'all 0.12s' }}>
      <div style={{ width: 18, height: 18, borderRadius: 4, border: `2px solid ${selected ? C.accent : C.border}`, background: selected ? C.accent : C.white, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 1 }}>
      ...
      <div>
        ...
        {eg && <div style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>{eg}</div>}
      </div>
    </div>
  )
}
...
function ScoreBar({ score }) {
  ...
  const grade = score >= 14 ? 'A' : score >= 9 ? 'B' : score >= 5 ? 'C' : 'D'
  const col = grade === 'A' ? C.teal : grade === 'B' ? C.blue : grade === 'C' ? C.warn : C.danger
  return (
    <div style={{ marginTop: 20, padding: '14px 16px', background: C.paper, borderRadius: 10 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
        <span style={{ fontSize: 12, color: C.muted, fontWeight: 500 }}>Live deal score</span>
        <span style={{ fontSize: 12, fontWeight: 700, color: col }}>{score}/{max} → Grade {grade}</span>
      </div>
...
export default function DemoForm() {
  ...
  useEffect(() => {
    if (prefilledDealId) {
      ...
      const autoVerify = async () => {
        ...
        try {
          const res = await authFetch(`/api/zoho/deal/${prefilledDealId}`);
  ...
  const verify = async () => {
    ...
    setLoading(true); setZohoErr("");
    try {
      const res = await authFetch(`/api/zoho/deal/${f.zohoId.trim()}`);
      const data = await res.json();
      if (!res.ok) { setZohoErr(data.error || "Deal not found in Zoho CRM."); return; }
      const resolvedName = typeof data.prospectName === 'object'
        ? (data.prospectName?.name || '')
  ...
  const handleSubmit = async () => {
    ...
    let syncResult;
    try {
      const syncRes = await authFetch('/api/deals/sync', { method: 'POST', body: JSON.stringify(f) });
      syncResult = await syncRes.json();
      if (!syncRes.ok) throw new Error(syncResult.error || 'Sync failed');
    } catch (err) {
    ...
    setSyncDone(true);
    setTimeout(() => navigate(`/pipeline/${syncResult.dealId}`), draftsWarning ? 2000 : 1500);
  };
