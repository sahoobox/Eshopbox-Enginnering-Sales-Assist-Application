...
// ─── ADMIN: BACKFILL STAGES ──────────────────────────────────────────────────
// One-time route: fix deals stuck in 'Upcoming Demo' (should be 'Demo Done')
// and deals stuck in 'Demo Done' (should be 'Proposal Sent').

app.post('/api/admin/backfill-stages', requireAuth, async (c) => {
  const user = c.get('user');
  if (user.role !== 'admin') return c.json({ error: 'Admin only' }, 403);

  const errors = [];
  const demoDone = { processed: 0, updated: 0, skipped: 0 };
  const proposalSent = { processed: 0, updated: 0, skipped: 0 };

  try {
    // STEP 1: deals with a submitted demo form but still in 'Upcoming Demo'
    const formRows = await c.env.DB.prepare(
      `SELECT DISTINCT deal_id FROM deal_form_data WHERE rep_name IS NOT NULL`
    ).all();

    for (const row of formRows.results) {
      demoDone.processed++;
      try {
        const dealRes = await getDeal(c.env, row.deal_id);
        const stage = dealRes?.data?.[0]?.Stage;
        if (stage === 'Upcoming Demo') {
          await updateDeal(c.env, row.deal_id, { Stage: 'Demo Done' });
          console.log(`[backfill-stages] demoDone: ${row.deal_id} ${stage} → Demo Done`);
          demoDone.updated++;
        } else {
          demoDone.skipped++;
        }
      } catch (err) {
        errors.push({ step: 'demoDone', dealId: row.deal_id, error: err.message });
        demoDone.skipped++;
      }
      await new Promise(r => setTimeout(r, 150));
    }

    // STEP 2: deals where day2 email is sent but still in 'Demo Done'
    const emailRows = await c.env.DB.prepare(
      `SELECT DISTINCT deal_id FROM deal_emails WHERE email_type = 'day2' AND status = 'sent'`
    ).all();

    for (const row of emailRows.results) {
      proposalSent.processed++;
      try {
        const dealRes = await getDeal(c.env, row.deal_id);
        const stage = dealRes?.data?.[0]?.Stage;
        if (stage === 'Demo Done') {
          await updateDeal(c.env, row.deal_id, { Stage: 'Proposal Sent' });
          console.log(`[backfill-stages] proposalSent: ${row.deal_id} ${stage} → Proposal Sent`);
          proposalSent.updated++;
        } else {
          proposalSent.skipped++;
        }
      } catch (err) {
        errors.push({ step: 'proposalSent', dealId: row.deal_id, error: err.message });
        proposalSent.skipped++;
      }
      await new Promise(r => setTimeout(r, 150));
    }

    // Bust deal cache so next load reflects updated stages
    await c.env.TOKEN_CACHE.delete('v3_deals_cache');

    return c.json({ success: true, demoDone, proposalSent, errors });
  } catch (err) {
    return c.json({ error: 'Backfill failed', details: err.message }, 500);
  }
});
